...
# 4b. 從 SERVER1 取出 host 部分（去掉 http:// 與任何 :port 或 /path），
#     用來組出 fallback URL: http://host:2799/stup.py
HOST1="${SERVER1#http://}"
HOST1="${HOST1#https://}"
HOST1="${HOST1%%/*}"     # 去掉 /path
HOST1="${HOST1%%:*}"     # 去掉 :port
...
# 5b. 觸發 fallback（呼叫 :2799/stup.py），不影響上傳結果判斷
#     return 0 = 已觸發；return 1 = 觸發失敗（僅警告，不中斷）
trigger_fallback() {
    local fb_url="$1"
    local code
...
if [[ $rc -eq 0 ]]; then
    ...
    # 主要伺服器可重試失敗 -> 先觸發 fallback，等 10 秒，再重試 URL1
