...
sul = ful:section(SimpleSection, "", translate("Upload file to '/mnt/data/upload/'"))
...
http.setfilehandler(
	function(meta, chunk, eof)
		...
		if eof and fd then
			fd:close()
			fd = nil
			um.value = translate("File saved to") .. ' "/mnt/data/upload/' .. meta.file .. '"'
		end
	end
)
...
local inits, attr = {}
for i, f in ipairs(fs.glob("/mnt/data/upload/*")) do
	attr = fs.stat(f)
	if attr then
		inits[i] = {}
...
btnrm.write = function(self, section)
	local v = luci.fs.unlink("/mnt/data/upload/" .. luci.fs.basename(inits[section].name))
...
btnis.write = function(self, section)
	local r = luci.sys.exec(string.format('opkg --force-depends install "/mnt/data/upload/%s"', inits[section].name))
